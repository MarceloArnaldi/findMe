 ...
 *   
 *   Versao 0.04 - Quando recebe umn comando de temporizacao, cancela outros timers (vTimerX = -1)
 *   Versao 0.05 - Mudanca da senha da rede AP de legoolus para legoolus22. Aparentemente a rede fica mais estavel com essa mudanca
 *   Versao 0.06 - Inclusao de digitalWrite(LED_1,DESLIGA);  depois da funcao BlinkLED()   
 *   Versao 0.07 - 2018 08 24 
 *   Versao 0.07 - Troca de lugar de atualizacao do dimAtual1/2/3 do inverteCanal() para acionaCanal()
 *   Versao 0.07 - cWebServer - quando o vClienteSSID_OK eh Ok vai direto para o cConnectToSSID
 *   Versao 0.07 - cErro - Reduzindo o tempo pela metade 
 *   Versao 0.07 - Quando WL_CONNECT_FAILED - Se NUNCA conectou na rede WIFI apaga as credenciais ELSE vai para o teclado possibilitando o RESET de fabribra
 ...
 *   Versao 0.07 - Possibilita o uso de 4 entradas - Feito para o cliente Roger
 *   Versao 0.08 - Trocado o valor vConnectSSID_Ok de 0 e 1 por OK e NT
 *   Versao 0.09 - No 'acionaSaida" quando LED RGB tem que ser "analogWrite" 
 *   Versao 0.10 - Em loadCredentials - o campo vConnectSSID_Ok = NT entao vClientSSID_OK = false estava ao contrario, sua ANTA
 *   Versao 0.11 - 2019 01 16 - MQTT e LKD2E3               
 *                 mandaFrameTX - LKD2E3 - Funcao para enviar o frame recebido para o ATMEL 
 *                 testeD2E3 - LKD2E3 - Liga Todas e Desliga Todas
 *                 callbackMQTT - MQTT - essa rotina eh ativada quando recebe alguma coisa dos itens assinados
 *   Versao 0.12 - 2019 01 17 - MQTT - Fazer com que topicos assinados sejam montados e nao fixos 
 *                 MQTT - Se o ID da MC nao estiver configurado nao assinar os comandos de Broadcast 
 *                 loadCredendial e SaveCredencial - Inclusao do ID da MC
 *                 MQTT - Inclusao topico TOPIC_SETIDMC - LK<Tipo-Modulo><ID-modulo>/SETIDMC/<ID da MC> - vide versao 0.15
 *                 MQTT - vConnectMQTT_OK - informa que houve uma conexao com sucesso ao MQTT
 *                 vBlink_OK - manda piscar os LEDs
 *                 vBlink_NotOK - manda piscar os LEDs alternados
 *                 Os LEDs piscam juntos por 3 vezes quando:
 *                 - Conecta na rede WIFI com sucesso
 *                 - Fixa o IP cm sucesso
 *                 - Conecta no sevidor MQTT com sucesso
 *   Versao 0.13 - 2019 03 21 - Ajuste na rotina - mandaFrameTX - antes recbia uma string e agora recebe 3 char
 *   Versao 0.14 - 2019 03 28 
 *                 LegoolusKitSmart - O App LegoolusKitSmart envia somente o SSID e PWD para o LK
 *                 Uma vez na rede WIFI o LK envia para o MQTT o seu IP assim o APP sabe onde esta o LK, armazena o IP e comunica via Socket quando a Internet nao esta disponivel
 *                 Com MQTT o IP dos LKs e da MC nao sao fixos
 *   Versao 0.15 - 2019 05 01 - Integracao com o APP
 *                 MQTT - Implementacao do Topico que nao sao via MC 
 *                 MQTT - Envio do sub-topico STATUS
 *                 MQTT - Tratamento do sub-topico GETSTATUS
 *                 Quando ha Reset de Fabrica o ESP eh resetado fisicamente   
 *                 MQTT - traduzFatorDimmer(int pFator) - Usado para montar o topico STATUS no MQTT 
 *                 MQTT - Envio do sub-topico SETIP - o LK invia o seu IP 
 *                 MQTT - Assinatura do sub-topico SETIDMC - a MC invia o seu ID
 *                 MQTT - Assinatura do sub-topico SETIPMC - a MC invia o seu IP
 *                 MQTT - Antes de Publicar ou Assinar verifica se o MQTT esta conectado
 *                 MQTT - Ao rodar pela primeira vez se nao conectar no servidor MQTT, continua o programa e tenta carregar em outro momento no cTeclado
 *                 MQTT - Envio do sub-topico SETIDLK - a MC invia o ID do LK na rede Legoolus
 *                 Mudanca da senha legoolus22 para legooluskit
 *                 Varios lugares - quando iguala uma String com "" - a variavel fica com SUJEIRA!
 *                 MQTT - Assinatura do sub-topico SETAPPTOKEN - a MC invia o seu IP
 *                 MQTT - Assinatura do sub-topico SETSCENE - o APP envia o cenario para a tecla 3 acionar
 *                 Switch cTeclado no loop() : Acinamentos padroes das telas :
 *                    R3E3 : Tecla 1 : inverte canal 1 - Tecla 2 intercala o canal 2 e 3 
 *                    D2E3 : Tecla 1 : inverte canal 1 e se mantem pressionada dimeriza canal 1 - Tecla 2 inverte canal 2 e se mantem pressionada dimeriza canal 2
 *                    Todos : Tecla 3 - desliga todas quando nao ha cenario ou aciona cenario 
 *                 Save e loadCredentials : Armazenando o ultimo valor dimerizado atraves do pulsador - esse valor eh usado no ON como limite
 *                 MQTT - enviaFrameMQTT - so envia frame de tecla pressionada quando o LK possuir ID
 *                 MQTT - enviaFrameMQTT - Topico de publicacao EVENT - envia o evento de tecla pressionada para o MQTT - LK02500033/EVENT/000000 - onde cada 0 ou 1 representa uma tecla pressionada - SEM USO - para uso futuro em alguma aplicacao sem MC
 *                 MQTT - publicaTopicStatusMQTT - não publica o STATUS se ele é o mesmo que o anterior - vLastStatus
 *                 MQTT - Implementado cenarios Broadcast Liga Todas e Desliga Todas - APP<ID do APP>/BROADCAST/F5FA00F120
 *  Versao 0.16 - 20/05/2019 - MQTT - TrataScene -  Inclusao do Liga Todas 
 *  Versao 0.17 - Versao final para Jornada de Automacao - MQTT Local na BBB by mosquitto
 *  Versoa 0.18 - 22/07/2019 - LK Shutter (34)
 *                LKShutter - vTempoShutter - Tempo de abertura e fechamento da cortina - configura via MQTT (comado ainda nao implementado)
 *                - Ao rodar o programa, se existe IP da MC, tenta conectar no MQTT Local, coloca a flag MQTTLocalOK como true e fica conectado no MQTT Local - Se nao entrontrar o MQTT local quando roda, nao tenta mais a conexao ate a proxima vez que ligar, por exemplo quando falta energia ou eh desligado propositalmente
 *                - Se houver MQTT Local toda comunicacao entre LK e MC sera pelo MQTT Local
 *                - Para a MC o MQTT Remoto serve so para receber comandos REMOTOS
 *                - Para a LK o MQTT Remoto serve para receber comandos REMOTOS e informar o mundo de seus atos
 *                - Se nao houver MQTT Remoto - tentar conexao a cada 5 mim
 *                - Quando o LK recebe o IP da MC, ele configura o MQTT Local
 * Versao 0.19  - 10/08/2019
 *              - Acesso dois servidores MQTT - Local MC e Remoto CLoudMQTT
 *              - uso do @ em Topicos de publicacao que usam o ID do LK para o LK nao ouvir o "eco"
 * Versao 1.00  - 24/09/2019 - Versao para venda numero 1 - Anibal - Restaurante DonDoh - 8 x LKRGB
 *              - Reducao do tempo ( blink ) de conexao no SSID e MQTT
 *              - Apos reset ou falta de energia - o LK volta na ultima posicao - vou tentar fazer isso somente quando ele reseta
 *              - Publica o STATUS mesmo se ele for igual ao anterior - versao 0.15
 *              - Em vez de publicar o STATUS direto -  aguardo um tempo ate receber o ultima solicitacao (interna ou externa) de envio de STATUS 
 * Versao 1.01  - Ajuste na rotina de recebimento do Socket quando configurado              
 * Versao 1.02  - Tentando resolver o problema de Estouro de Memoria - e quando perde conexao com a Internet as vezes trava
 *              - vTempoShutter armazena a ultima cor do RGB alem do tempo da cortina - quando RGB eh ligado ele volta para a ultima cor 
 * Versao 1.03  - Continuacao da 1.02 - A rotina cTeclado esta muito grande - vou transformar cara etapa em rotinas para limpar o codigo para deposis descobrir quem pode estar travando/estouro de pilha
 * Versao 1.04  - Quando houver valor em vTempoShutter nao executa teste de ligar os canais (TesteD2)
 *              - Troca o vTempoShutter por vValorAux
 *              - Exclusao suporte LK ER6
 * Versao 1.05  - Topico: ZERAMCINFO para zerar informacoes da MC no LK e assim nao acessar mais o MQTT local
 *              - Topico: ZERAALLINFO para zerar TODAS informacoes no LK e assim nao acessar mais o MQTT local
 *              - Quando desconecta o MQTT a forca nao grava as credenciais
 * Versao 1.06  - setIPMC - Quando recebe o IP da MC - so Grava as credenciais se o IP for valido e se for diferente do armazenado
 *              - analisaFrameATMEL - quando recebe um comando de INVERSAO, quando o ATMEL nao terminar o fadein ou fadeout nao pode enviar outro comando INVERTE no mesmo canal porque o ATMEL nao aceita e o STATUS fica errado
 * Versao 1.07  - CORRECAO ERRO - BROACAST - comando broadcast quando dimeriza um modulo ou dimeriza todos os modulos o byte de dado eh F0 e o LK entedia que era comnado Desliga Todas e nao dimriza              
 *              - Implementacao do comnado F5 FA 70 F0/0F - Incrementa e Decrementa TODOS os dimmers
 *              - Implementacao do comnado F5 FA MM F0/0F - Incrementa e Decrementa todos os canais de UM modulo dimmer
 *              - Exclusao do tratamento de comandos via socket
 *              - Unificacao do Callcabk MQTT local e remoto
 *              - testaMQTT - aumento de 10 s para 20 s entre as tentativas de conexao com MQTT Remoto
 * Versao 1.08  - Finalizando Shutter - quando recebe comandon para acionar o canal, ele eh temporizado usabndo a variavel vValorAux - a rotina "temporizaCanal" de temporizacao considera cada unidade do valor enviado por 110ms (mesmo padrao usado nos modulo cabeados) 
 *              - Finalizando Shutter - tratamento para o comando MQTT para "setar" o valor de abertura e fechamento da cortina - default eh 30 seg - Topico MQTT 
 *              - Finalizando Shutter - Topico para alterar o tempo: TOPIC_SETSHUTTER = "/SETSHUTTER@";
 *              - Finalizando Shutter - No STATUS os terceiro elemento informa se a cortina esta aberta ou fechada -  se o modulo desliga ele perde essa informacao, podeiria armazenar essa informacao mas nao quero
 * Versao 1.09  - Tentativas para nao travar a cominucacao do MQTT
 *              - Ate entao existe um time que desconecta o MQTT e coneca novamente - Rotina : testaMQTT - MQTTcount - nao desconecta mais
 *              - Reset de Fabrica - quando nao conecta no MQTT o time ficava menor - nao mais
 * Versao 1.10  - cTempoShutter - armazena o tempo de acionamento da cortina - era 30 seg - agora 180 seg             
 * Versao 1.11  - Ao não conseguir conectar no servidor MQTT mosquitto local - simplesmente para de tentar até rececer novamento o IP da central 
 *              - MQTTLocalCount - controlava a conexao
 *              - MQTTLocalOk - informa se existe IP da MC - se nao conseguir conectar na MC agora eh colocado FALSE para NAO tentar novamente
 * 
 * 
 * STATUS de erro do MQTT
 * 
...
String vNS = "00008"; // RGB 
//String vNS = "00002"; // R3E3
//String vNS = "00002"; // Led Strip // 00009
//String vNS = "00091"; // Shutter
...
/*
int           vContBloqueiaInverteD2_1  = 0;
int           vContBloqueiaInverteD2_2  = 0;
boolean       vBloqueiaInverteD2_1      = false;
boolean       vBloqueiaInverteD2_2      = false;
*/
// Cenarios
//
typedef struct 
...
//int resetFabrica = gpio0_pin; // Reset de Fabrica - usar essa quando o fio do pulsador 3 quebrado
//int resetFabrica = gpio2_pin; // Reset de Fabrica - usar essa quando o fio do pulsador e 1 e 3 quebrados

//
// Saidas - Rele R3E3 - Led - Shutter
//
// Por definicao os dois canais do extremo serao 1 e 2, o do meio sera 3
//
#define SAIDA_1 05
#define SAIDA_2 14 // antigo 12
#define SAIDA_3 12 // antigo 14
//

volatile byte estado_p0 = LOW;
volatile byte estado_p2 = LOW;
volatile byte estado_p4 = LOW;
//volatile byte estado_p5 = LOW;
//volatile byte estado_p12 = LOW;
//volatile byte estado_p14 = LOW;
volatile byte ep0 = LOW;
volatile byte ep2 = LOW;
volatile byte ep4 = LOW;
//volatile byte ep5 = LOW;
//volatile byte ep12 = LOW;
//volatile byte ep14 = LOW;
...
void setup()
{
  ...
  vMsgInicial.concat(" - Versao 1.11 - Data : 26/08/2020 - ns : ");
  vMsgInicial.concat(cTpNS);
  vMsgInicial.concat(vNS);
    ...
    if (!MQTTLocal.connected()) { 
      ...
      if (MQTTLocalCount > 3100000) {
        ...
        if (MQTTLocal.connect(CLIENT_MQTT, BROKER_USER, BROKER_PASSWORD)) {
            ...
            // 1.11 - nao tentar conectar local apos o primeiro fracasso - o MQTTLocalOk informa que nao tem o IP da MC - so voltara a ser True quando recenber om IP da MC ou quando recetar o modul
