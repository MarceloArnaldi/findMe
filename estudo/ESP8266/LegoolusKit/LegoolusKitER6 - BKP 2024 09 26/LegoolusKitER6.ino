 /*
 *   
 *   LegoolusKit ER6 - O ER6 por ter 6 entradas de pulsador nao foi possivel usar o projeto LegoolusKitSmart por os GPIOs nos outros modulos sao usados para SAIDA
 *   
 *   Reset de Fabrica : Pressionar o tecla 3 por 30 segundos
 *   
 ...
 *   WiFiManager - biblioteca ESP8266 - versao 0.12.0
 *   
 ...
 * Versao 1.04  - Quando houver valor em vTempoShutter nao executa teste de ligar os canais (TesteD
 ...
 * Versao 1.08  - O LegoolusKitSmart foi clonado e ajustado para 6 entradas e 0 saidas
 * 
*/
...
// ER6
#define             cTipo         21
const char          *cTipoModulo  = "ER6"; 
const String        cTpNS         = "021"; 
...
String        vValorAux                 = "000000000"; // Antigo vTempoShutter
//
...
// TECLADOS
//
// Versao 1.08 - Todos os modulos tem 3 teclas - LKER6 tem 6 teclas
// R3E3 - 0 2 4 --> Existe o conector verde com as portas 5 12 14 - como o D2 nao tem conector verde, para mim essa placa nao tem conector verde - afinal Para que padronizar ?? 
...
// Versao 1.08 - Exclusao do suporte as saidas
//
//#define SAIDA_1 05
...
// Versao 1.08 - inclusao de 6 entradas 
...
int         MQTTLocalCount     = 3100000; // Temporiza as tentativas de conexao ao Broker MQTT Local - ele comeca com valor para tentar a conexao na primeira vez que passa pela verificacao
...
void setup()
{
  ...
  vMsgInicial.concat(" - Versao 1.08 - Data : 18/12/2019 - ns : ");
  vMsgInicial.concat(cTpNS);
  vMsgInicial.concat(vNS);
  ...
  if (cTipo != LKD2E3) {
    ...
    if (cTipo != LKSHUTTER) {
      // 1.04 - Se for RGB nao pode ter valor armazenado no vTempoShutter/vValorAux
...
void nivel () {
  ...
  // versao 1.08 - Inclusao suporte LK ER6
...
void testaMQTT() {
  ...
  if (!MQTT.connected()) {
    ...
    if (MQTTCount > 1000000) {
      MQTTCount = 0;            
      debugln(" ");
      debug("Tentando Conectar no MQTT remoto : ");
    ...
    if (!MQTTLocal.connected()) { 
      ...
      if (MQTTLocalCount > 3100000) {
        ...
        if ((MQTTLocalCount % 51000) == 0) {  
          ...
          if ((MQTTCount % 100000) == 0) {  debug("ml ");  }
        }
        MQTTLocalCount += 1; 
      }
    } else { MQTTLocal.loop(); }
